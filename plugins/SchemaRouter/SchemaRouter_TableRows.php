    ...
    if(
      ...
      if($Column['IsConstraint']['PRIMARY KEY']){
        ...
        if($Column['IsConstraint']['FOREIGN KEY']){
          ...
          $SQL.="CONCAT('<a href=\"/".$Schema."/".Sanitize($ForeignTable)."/',`".Sanitize($Column['COLUMN_NAME'])."`,'\">',(SELECT `".Sanitize($ForeignObjectName)."` FROM `".Sanitize($ForeignTable)."` WHERE `".Sanitize($ForeignTable)."`.`".Sanitize($ForeignObjectName)."` = `".Sanitize($Table)."`.`".Sanitize($Column['COLUMN_NAME'])."`),'</a>') as '".Sanitize($ForeignObjectName)."',";
