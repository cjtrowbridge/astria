...
function SchemaRouter_RowColumns_Fields_BodyCallback_ForeignKey($Schema,$Column,$Value){
  ...
  if(!(isset($Description[0]))){echo '<p>Unable to locate reference field for object.</p>';pd($Description[0]);}
  if(!(isset($Description[0][ Sanitize($FirstTextField) ]))){echo 'Unable to locate reference field for object.';pd($Description[0]);}
