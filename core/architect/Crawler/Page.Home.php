...
function Architect_Crawler_Homepage_BodyCallback(){
  ...
  <?php 
    $Crawlers = Query('SELECT *,(SELECT COUNT(*) FROM CrawlerTask WHERE CrawlerTask.CrawlerID = Crawler.CrawlerID AND CrawlerDataset.CrawlerID = CrawlerTask.CrawlerID) as "Tasks" FROM Crawler LEFT JOIN CrawlerDataset ON Crawler.CrawlerID = CrawlerDataset.CrawlerID');
